...
if __name__ == '__main__':
    image_paths = '/media/data1/sivankeret/ocr_datasets/Arabic/ifn_enit_synth/quran_synth/DecoType_Naskh_Special/*.png'
    out_path = '/media/data1/sivankeret/ocr_datasets/Arabic/ifn_enit/trans_test'
    os.makedirs(out_path, exist_ok=True)

    for i, image_path in enumerate(glob.iglob(image_paths)):
        if i > 10:
            break
        rand_val = np.random.rand(1)
        rand_axis = np.random.randint(2)
        image = cv2.imread(image_path)
        image = {"img": image}
        base_name = basename(image_path)
        input_height = 64
        resize = Resize(hight=input_height)
        image = resize(image)
        base_name = basename(image_path)
        cv2.imwrite(join(out_path, base_name + '_0.png'), image["img"])
        elatic = OnlyElastic(40, 3)
        image = elatic(image)
        cv2.imwrite(join(out_path, base_name + '_1.png'), image["img"])
        sine = OnlySine(5)
        image = sine(image)
        cv2.imwrite(join(out_path,base_name+'_2.png'), image["img"])
        rot = Rotation(2)
        image = rot(image)
        cv2.imwrite(join(out_path, base_name + '_3.png'), image["img"])
        color_grad = ColorGrad()
        image_tmp = color_grad(image.copy())
        cv2.imwrite(join(out_path, base_name + '_4.png'), image_tmp["img"])
        gn = ColorGradGausNoise()
        image = gn(image)
        cv2.imwrite(join(out_path, base_name + '_5.png'), image["img"])
        bw = ToBW()
        image = bw(image)
        cv2.imwrite(join(out_path, base_name + '_6.png'), image["img"])
